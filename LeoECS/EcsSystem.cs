...
namespace Leopotam.Ecs {
    ...
    public sealed class EcsSystems : IEcsInitSystem, IEcsDestroySystem, IEcsRunSystem {
        ...
        /// <summary>
        /// Processes all IEcsPhysicRunSystem systems.
        /// </summary>
        public void RunPhysics () {
            ...
            if (_destroyed) { throw new Exception ("Cant touch after destroy."); }
            #endif
            for (int i = 0, iMax = _runPhysicSystems.Count; i < iMax; i++) {
                var runItem = _runPhysicSystems.Items[i];
                if (runItem.Active) {
                    runItem.System.RunPhysics ();
                }
                #if DEBUG
                if (World.CheckForLeakedEntities (null)) {
    ...
    /// <summary>
    /// IEcsPhysicRunSystem instance with active state.
    /// </summary>
